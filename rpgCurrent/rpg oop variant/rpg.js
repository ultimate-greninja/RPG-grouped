...
class Enemy{
    constructor(enemyType,enemyMaxHealth,enemyCurrentHealth,enemyAttack,enemyAttackSpeed){
        ...
        this.enemyTurnsUntilAttack = 0 //40 = 1 turn 2.5 7?
        //2.5 = 7 5 = 6 10 = 4 20 = 3 30 = 2 40 = 1
    }
    TurnIndicator(enemyAttackSpeed){
        let enemyTurnsUntilAttack = 7
        let enemyAttackSpeedCount = enemyAttackSpeed
        while (enemyAttackSpeedCount != 2.5 & enemyAttackSpeedCount > 0){
            if (enemyAttackSpeedCount > 10){
                enemyAttackSpeedCount = enemyAttackSpeedCount - 10
            }
            else{
                enemyAttackSpeedCount = enemyAttackSpeedCount/2
            }
            enemyTurnsUntilAttack--
        }
        if (enemyTurnsUntilAttack < 1){
            console.log("error, enemyTurnsUntilAttack Start calculator went too low")
        }
        this.enemyTurnsUntilAttack = enemyTurnsUntilAttack
    }
    EnemyHealth(){
        let expid = document.querySelector("#exp")
        let userLvid  = document.querySelector("#userLv")
...
class Weapon{
    constructor(weaponName,weaponRarity,weaponRarityNumber,weaponPower,weaponAttackSpeed,weaponDurability,effect,curse,worthyness,sellPrice){
        ...
        this.maxWeaponDurability = maxWeaponDurability
    ...
    weaponRoll(){
        ...
        if (userGold >= weaponPullCost){
            ...
            if (itemRarity.fail == true){
                ...
                let itemMaxWeaponDurability = itemWeaponDurability
                let itemWeaponName =""
                let curseName = ""
                ...
                let weapon = {itemWeaponName,itemWeaponRarity,itemTypeStore,itemWeaponPower,itemAttackSpeed,itemMaxWeaponDurability,itemWeaponDurability,effect,curses,itemSellPrice}
                ...
                setCookie("maxWeaponDurability",`${weapon.itemMaxWeaponDurability}`,1)
...
class Game{//for the attacks
    ...
    swing(){
        ...
        if (this.Game.enemy.enemyTurnsUntilAttack == 0){
            //enemy attack here
        }
        else{
            this.Game.enemy.enemyTurnsUntilAttack--
        }
    }
