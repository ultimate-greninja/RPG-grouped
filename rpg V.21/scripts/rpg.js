...
function weaponRoll(){
    ...
    if (userGold >= weaponPullCost){
        ...
        if (itemRarity.fail == true){
            ...
            let weaponData = {
                weaponRarity:`${weapon.itemWeaponRarity}`,
                typeStore: `${weapon.itemTypeStore}`,
                weaponPower: `${weapon.itemWeaponPower}`,
                attackSpeed: `${weapon.itemAttackSpeed}`,
                weaponDurability: `${weapon.itemWeaponDurability}`,
                effect: `${weapon.effect}`,
                curse: `${weapon.curses.curse}`,
                worthyness: `${weapon.curses.unworthy}`,
                sellPrice: `${weapon.itemSellPrice}`
            }
            console.log(JSON.stringify(weaponData))//weaponData fine
...
function userHealth(){
    ...
    let maxhealthid = document.querySelector("#maxUserHealth")
    let maxhealth = parseInt(maxhealthid.innerHTML)

    let healthid = document.querySelector("#userHealth")
    let health = parseInt(healthid.innerHTML)
    health = 50 + ((userLv * userLv) * 50)
    healthid.innerHTML = health
    maxhealthid.innerHTML = health
}
...
let userGoldid = document.querySelector("#userGold")
while (WR !== "Universal" && weaponPick == true){
    weaponRoll()
...
function enemyTurnIterative(){
    ...
    if (SAid.innerHTML == "true"){
        ...
        enemySpeed = (100/enemySpeed)*1000
