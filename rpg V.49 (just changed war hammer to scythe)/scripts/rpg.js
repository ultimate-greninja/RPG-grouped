...
function Death(){
    ...
    // change to send to death.php or cookie that sends to death.php
}


//weapon type

function weaponType(weaponPower,weaponDurability,itemSellPriceMultiplier){
    let attackSpeed = 10
...
function weaponRoll(){
    ...
    if (userGold >= weaponPullCost){
        ...
        weaponRollid.innerHTML = `<input type="button" onclick= "weaponRoll()" value="Weapon roll" hidden/>`
        ...
        if (itemRarity.fail == true){
            ...
            let itemType = weaponType(itemRarity.weaponPower,itemWeaponDurability,itemSellPriceMultiplier)
...
let userGoldid = document.querySelector("#userGold")
while ((WR !== "Legendary") && weaponPick == true){
    weaponRoll()
